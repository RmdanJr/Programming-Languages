...
fun number_before_reaching_sum(sum : int, nums: int list) =
  let 
    fun get_n(nums : int list, target : int, total : int, n : int) =
        if total + (hd nums) < target 
        then get_n(tl nums, target, total + (hd nums), n + 1)
        else n
  in get_n(nums, sum, 0, 0)
  end


(*Write a function what_month that takes a day of year (i.e., an int between 1 and 365) and return what month that day is in (1 for January, 2 for February, etc.). Use a list holding 12 integers and your answer to the previous problem*)
